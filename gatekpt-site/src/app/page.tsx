...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setResult(data.transcription||data.error||'Oops');
    } catch {
      setResult('Error');
    } finally {
      setLoading(false);
  ...
  return (
    <>
      ...
      <a href="#main-content" className="sr-only focus:not-sr-only fixed top-4 left-4 z-50
                                        px-3 py-2 bg-white text-black rounded">
        Skip to main content
      </a>

      {/* Hero + Chat */}
      <div className="flex flex-col items-center space-y-8">
        <h1 className="text-[5rem] md:text-[6rem] font-[Cinzel] font-extrabold uppercase
      ...
      {/* Tagline */}
      <p className="mt-12 mb-8 max-w-xl text-center text-gray-300 font-light
                    animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row gap-8 w-full justify-center mb-12">
        ...
        <Link href="/downloads/windows">
          <button className="px-8 py-3 rounded-full border border-white text-white
                             font-semibold uppercase hover:bg-white hover:text-black
                             hover:shadow-[0_0_30px_rgba(212,175,55,1)] transition">
      ...
      <form id="main-content" onSubmit={handleSubmit}
        ...
        <label htmlFor="audioFile" className="sr-only">Upload audio file</label>
        <input id="audioFile" type="file" name="audioFile" accept="audio/*" required
               className="w-full bg-black/30 text-white file:bg-white file:text-black
                          file:px-4 file:py-2 file:rounded-lg file:cursor-pointer
                          border border-white/20 rounded-lg focus:outline-none
                          focus:ring-2 focus:ring-white/50 transition"
        />
        <button type="submit" disabled={loading}
                className="px-6 py-3 rounded-full bg-white text-black font-semibold uppercase
